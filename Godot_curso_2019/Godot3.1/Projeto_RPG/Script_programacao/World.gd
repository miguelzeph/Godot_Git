...
onready var cur_chunk = get_node("YSort_Chunk0x0")
...
func switch_chunk(from_chunk, delta):
	if switching: return 
	switching = true
	#self.from_chunk = pego a variavel global... from_chunk = pego a variavel da funcao
	self.from_chunk = from_chunk

	var to_index = from_chunk.index + delta
	var ckstr = cur_world+"_"+str(to_index.x)+"x"+str(to_index.y)
	
	var pre_chunk = load("res://Cenas_do_Projeto/"+cur_world+"/"+ckstr+".tscn")
